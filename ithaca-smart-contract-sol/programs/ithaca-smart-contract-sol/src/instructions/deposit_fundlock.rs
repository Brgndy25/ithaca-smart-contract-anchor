...
pub struct DepositFundlock<'info> {
    ...
    #[account(
        init_if_needed,
        payer = client, 
        seeds = [b"client_balance_state".as_ref(), fundlock.key().as_ref(), client_ata.key().as_ref()],
    ...
    #[account(
        init_if_needed,
        payer = client,
        seeds = [b"client_balance".as_ref(), fundlock.key().as_ref(), client_balance_state.key().as_ref()],
        token::mint = token,
